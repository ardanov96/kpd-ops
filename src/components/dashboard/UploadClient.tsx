...
const DEFAULT_EKSPEDISI = [
  ...
  { nama: 'JNE Express', kode: 'JNE', warna: '#ef4444' },
...
export default function UploadClient({ logs }: { logs: any[] }) {
  ...
  async function fetchEkspedisi() {
    ...
    try {
      const res = await fetch('/api/ekspedisi', { cache: 'no-store' })
    ...
    return (
      <div style={{ display: 'flex', gap: 6, justifyContent: 'center', marginTop: 12 }}>
        <button onClick={() => onChange(page - 1)} disabled={page <= 1} style={{
          background: '#1e2433', border: 'none', borderRadius: 5, padding: '4px 10px',
        ...
        {Array.from({ length: total }, (_, i) => i + 1).map(p => (
          <button key={p} onClick={() => onChange(p)} style={{
            ...
            color: p === page ? '#fff' : '#94a3b8', cursor: 'pointer', fontSize: 12,
            fontWeight: p === page ? 700 : 400,
          }}>{p}</button>
        ))}
        <button onClick={() => onChange(page + 1)} disabled={page >= total} style={{
          background: '#1e2433', border: 'none', borderRadius: 5, padding: '4px 10px',
          color: page >= total ? '#2d3748' : '#94a3b8', cursor: page >= total ? 'not-allowed' : 'pointer', fontSize: 12,
        }}>→</button>
      </div>
    )
  }
  ...
  return (
    <div style={{ padding: 28 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '400px 1fr', gap: 24, alignItems: 'start' }}>
        ...
        {/* ── Kolom kanan: Riwayat + Kelola Ekspedisi ── */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>

          {/* Riwayat Import */}
          <div className="card" style={{ padding: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: '#94a3b8' }}>🕐 Riwayat Import</div>
              {logs.length > 0 && (
                <span style={{ fontSize: 11, color: '#475569' }}>{logs.length} total · hal {logPage}/{totalLogPages}</span>
              )}
            </div>
            {logs.length === 0 ? (
              ...
              <>
                    ...
                    return (
                      <div key={log.id} style={{ background: '#0d111c', borderRadius: 10, padding: '12px 14px', border: '1px solid #1e2433' }}>
                        ...
                        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                          <div style={{ flex: 1, background: '#1e2433', borderRadius: 4, height: 3, overflow: 'hidden' }}>
                            <div style={{ width: `${(log.success_rows / Math.max(log.total_rows, 1)) * 100}%`, height: '100%', background: '#22c55e' }} />
                          </div>
                          <span style={{ fontSize: 10, color: '#475569', whiteSpace: 'nowrap' }}>
                            {((log.success_rows / Math.max(log.total_rows, 1)) * 100).toFixed(0)}%
                          </span>
                        </div>
                      </div>
                    )
                ...
                <Pagination page={logPage} total={totalLogPages} onChange={setLogPage} />
              </>
            )}
          </div>
          ...
          <div className="card" style={{ padding: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                {ekspedisiList.length > 0 && (
                  <span style={{ fontSize: 11, color: '#475569' }}>{ekspedisiList.length} ekspedisi</span>
                )}
            ...
            {loadingEkspedisi ? (
              ...
              <>
                ...
                <Pagination page={eksPage} total={totalEksPages} onChange={setEksPage} />
              </>
            )}
          </div>
        </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 110px', gap: 12 }}>
                <div>
                  ...
                  <input style={inp} value={form.nama} onChange={e => setForm(f => ({ ...f, nama: e.target.value }))} placeholder="cth: Lion Parcel" />
                </div>
                <div>
                  <label style={lbl}>Kode *</label>
                  <input style={{ ...inp, textTransform: 'uppercase' }} value={form.kode}
                    onChange={e => setForm(f => ({ ...f, kode: e.target.value.toUpperCase() }))} placeholder="LION" maxLength={8} />
                </div>
              </div>
              ...
              <div>
                ...
                <div style={{ marginTop: 10 }}>
                  <span style={{
                    background: `${form.warna}25`, color: form.warna, border: `1px solid ${form.warna}60`,
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  ...
                  <input style={inp} value={form.telepon} onChange={e => setForm(f => ({ ...f, telepon: e.target.value }))} placeholder="08xx..." />
                </div>
                <div>
                  <label style={lbl}>URL Portal Laporan</label>
                  <input style={inp} value={form.portal_url} onChange={e => setForm(f => ({ ...f, portal_url: e.target.value }))} placeholder="https://..." />
                </div>
              </div>

              <div>
                <label style={lbl}>Keterangan</label>
                <input style={inp} value={form.keterangan} onChange={e => setForm(f => ({ ...f, keterangan: e.target.value }))} placeholder="Catatan tambahan..." />
              </div>
